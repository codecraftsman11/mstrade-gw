...
class AsyncBinanceApiClient(BaseBinanceApiClient):
    ...
    async def get_server_time(self, **kwargs) -> dict:
        ...
        return await self._request(method, url, signed, data=kwargs)

    async def stream_get_listen_key(self, **kwargs) -> str:
        method, url, signed = self.get_method_info('stream_get_listen_key')
        resp = await self._request(method, url, signed, data=kwargs)
    ...
    async def futures_stream_get_listen_key(self, **kwargs) -> str:
        ...
        resp = await self._request(method, url, signed, force_params=True, data=kwargs)
        return resp['listenKey']
    ...
    async def futures_ping(self, **kwargs) -> dict:
        ...
        return await self._request(method, url, signed, force_params=True, data=kwargs)
    ...
    async def get_futures_coin_position_info(self, **kwargs) -> List[dict]:
        ...
        if symbol := kwargs.pop('symbol', None):
            ...
            return [position for position in await self._request(method, url, signed, force_params=True, data=kwargs)
