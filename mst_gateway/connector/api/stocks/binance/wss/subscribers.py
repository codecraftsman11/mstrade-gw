...
class BinanceWalletExtraSubscriber(BinanceWalletSubscriber):
    ...
    async def get_state(self, api: BinanceWssApi, client: rest.BinanceRestApi):
        schema_handlers = {
            ...
            OrderSchema.margin: (client.handler.get_futures_account, utils.load_futures_wallet_extra_state)
        }
        try:
            kwargs = {'raw_data': client.handler.handle_response(schema_handlers[api.schema][0]())}
        except (GatewayError, BinanceAPIException):
            return {}
        if api.schema in (OrderSchema.margin,):
            try:
                cross_collaterals = client.handler.handle_response(client.handler.get_futures_loan_wallet())
            except (GatewayError, BinanceAPIException):
                cross_collaterals = {}
            kwargs['cross_collaterals'] = utils.load_margin_cross_collaterals_data(cross_collaterals)
        return schema_handlers[api.schema][1](**kwargs)
