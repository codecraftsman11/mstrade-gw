...
async def _wss_api() -> BinanceFuturesWssApi:
    with BinanceFuturesWssApi(name=cfg.BINANCE_WSS_API_NAME, account_name=cfg.BINANCE_ACCOUNT_NAME,
                              schema=cfg.BINANCE_FUTURES_SCHEMA, auth=cfg.BINANCE_AUTH_KEYS,
...
class TestBinanceFuturesWssApi:
    ...
    #     self, _wss_api: BinanceFuturesWssApi, subscr_name, symbol, subscr_channel, subscriptions
    # ):
    #     self.reset()
    #     assert not self.data
    ...
    #
    # @pytest.mark.asyncio
    # async def test_binance_wss_futures_unsubscribe(self, _wss_api: BinanceFuturesWssApi):
    #     _wss_api._subscriptions = {
    #         "symbol": {"btcusdt": {"1"}, "ethusdt": {"1", "2"}}, "order": {"*": {"1", "2"}}
    #     }
    ...
    #
    # @pytest.mark.asyncio
    # async def test_binance_wss_futures__restore_subscriptions(self, _wss_api: BinanceFuturesWssApi):
    ...
    # ])
    # def test_binance_wss_futures_get_state(self, _wss_api: BinanceFuturesWssApi, subscr_name, symbol):
    #     assert _wss_api.get_state(subscr_name, symbol) is None
    #
    # @pytest.mark.parametrize("symbol", [None, "not exists"])
    # def test_binance_wss_futures_get_state_data_is_none(self, _wss_api: BinanceFuturesWssApi, symbol):
    #     assert _wss_api.get_state_data(symbol) is None
    #
    # @pytest.mark.parametrize("symbol", ["btcusdt", "BTCUSDT"])
    # def test_binance_wss_futures_get_state_data(self, _wss_api: BinanceFuturesWssApi, symbol):
    ...
    # ])
    # def test_binance_wss_futures_parse_message(self, _wss_api: BinanceFuturesWssApi, message, result):
    ...
    # ])
    # def test_binance_wss_futures__lookup_table(self, _wss_api: BinanceFuturesWssApi, message, result):
    ...
    def test_binance_wss_futures_split_order(self, _wss_api: BinanceFuturesWssApi):
        # Save data for the tests below because of message.pop("data") in split_order(message)
        data = FUTURES_ORDER_LOOKUP_TABLE_RESULT["data"]
        assert _wss_api._split_message(
            FUTURES_ORDER_LOOKUP_TABLE_RESULT) == FUTURES_ORDER_SPLIT_MESSAGE_RESULTS
        FUTURES_ORDER_LOOKUP_TABLE_RESULT["data"] = data

    @pytest.mark.parametrize("message, split_results", [
        (FUTURES_ORDER_BOOK_LOOKUP_TABLE_RESULT, FUTURES_ORDER_BOOK_SPLIT_MESSAGE_RESULTS),
        (FUTURES_QUOTE_BIN_LOOKUP_TABLE_RESULT, FUTURES_QUOTE_BIN_SPLIT_MESSAGE_RESULTS),
    ...
    def test_binance_wss_futures__split_message(self, _wss_api: BinanceFuturesWssApi, message, split_results):
    ...
    def test_binance_wss_futures_split_wallet(self, _wss_api: BinanceFuturesWssApi):
        # Save data for the tests below because of message.pop("data") in split_wallet(message)
        data = FUTURES_WALLET_LOOKUP_TABLE_RESULT["data"]
        assert _wss_api._split_message(
            FUTURES_WALLET_LOOKUP_TABLE_RESULT) == FUTURES_WALLET_SPLIT_MESSAGE_RESULTS
        FUTURES_WALLET_LOOKUP_TABLE_RESULT["data"] = data

    @pytest.mark.parametrize("messages, results", [
        (FUTURES_ORDER_BOOK_SPLIT_MESSAGE_RESULTS, FUTURES_ORDER_BOOK_GET_DATA_RESULTS),
        (FUTURES_ORDER_SPLIT_MESSAGE_RESULTS, FUTURES_ORDER_GET_DATA_RESULTS),
        (FUTURES_QUOTE_BIN_SPLIT_MESSAGE_RESULTS, FUTURES_QUOTE_BIN_GET_DATA_RESULTS),
    ...
    def test_binance_wss_futures_get_data(self, _wss_api: BinanceFuturesWssApi, messages, results):
        for i, message in enumerate(messages):
            assert _wss_api.get_data(message) == results[i]

    def test_binance_wss_futures_get_data_wallet(self, _wss_api: BinanceFuturesWssApi):
        # Save data for the tests below because of state_data.pop('balances', {})
        # in BinanceFuturesWalletSerializer._wallet_detail(self, item, state_data),
        # where state_data are the user wallet storage data
        wallet_storage_data = _wss_api.storage.get(f"wallet.{_wss_api.account_name}")
        for i, message in enumerate(FUTURES_WALLET_SPLIT_MESSAGE_RESULTS):
            assert _wss_api.get_data(message) == FUTURES_WALLET_GET_DATA_RESULTS[i]
        _wss_api.storage.set(f"wallet.{_wss_api.account_name}", wallet_storage_data)
    ...
    @pytest.mark.asyncio
    async def test_binance_wss_futures_process_wallet_message(self, _wss_api: BinanceFuturesWssApi):
        print(_wss_api.storage.get(f"wallet.{_wss_api.account_name}"))
        _wss_api._subscriptions = {"wallet": {"*": {"1"}}}
        self.reset()
        assert not self.data
        await _wss_api.process_message(json.dumps(FUTURES_WALLET_MESSAGE), self.on_message)
