...
deploy_tag_variable=${DEPLOY_TAG:-'BUILD_MASTER_TAG'}
cd ..
...
v_uuid=$(curl -s -X GET -H "Content-Type: application/json" -H "Authorization: Bearer ${bb_token}"  "${deployrepo}/pipelines_config/variables/?pagelen=99" | jq -r ".values | .[] | select( .key == \"${deploy_tag_variable}\" ) | .uuid | @uri")
...
cmds+=("curl -s -X POST -H \"Content-Type: application/json\" -H \"Authorization: Bearer ${bb_token}\" --data '{ \"key\": \"${deploy_tag_variable}\", \"value\": \"${gittag}\", \"secured\": false}' \"${deployrepo}/pipelines_config/variables/\"")
